...
# the same block-causal structure into a DENSE boolean mask and run
# torch.nn.functional.scaled_dot_product_attention (see _neuron_flex_attention
# and _dense_mask_from_fn below). Numerics are equivalent: flex's default
# softmax scale is 1/sqrt(head_dim), which is also SDPA's default.
...
def _dense_mask_from_fn(mask_fn, q_len, kv_len, device):
    """Neuron flex_attention replacement (mask build).

    Materialize the block-causal ``mask_fn`` (the SAME closure the upstream code
    passed to flex_attention's ``create_block_mask``) into a DENSE boolean mask
    of shape ``[q_len, kv_len]`` where ``True`` means "attend". The upstream
    ``mask_fn(b, h, q_idx, kv_idx)`` returns True where attention is allowed and
    uses only vectorizable ops (comparisons + gathers like ``ends[q_idx]``), so
    we can evaluate it once on broadcastable index grids instead of per-element.

    Every upstream predicate OR-s in ``q_idx == kv_idx`` (the diagonal), so no
    row is ever all-False — this guarantees SDPA's softmax never sees a fully
    masked row (which would produce NaN), including in the right-padding region.
    """
...
def _neuron_flex_attention(query, key, value, block_mask):
    """Neuron flex_attention replacement (attention).

    Drop-in for the upstream ``flex_attention(query, key, value, block_mask=...)``
    calls. Inputs are ``[B, H, L, D]`` (already transposed by the caller).
    ``block_mask`` is the dense boolean ``[Lq, Lk]`` mask from
    ``_dense_mask_from_fn`` (True = attend); it broadcasts over B and H. We pass
    it straight to SDPA as ``attn_mask`` with ``is_causal=False`` because the
    dense mask already encodes the (block-)causal structure. Returns
    ``[B, H, L, D]`` like flex_attention did.
    """
    return F.scaled_dot_product_attention(
        query, key, value, attn_mask=block_mask, is_causal=False)
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if is_tf:
                ...
                # Neuron: flex_attention -> SDPA with a dense boolean mask.
                # block_mask is now the dense [Lq, Lk] bool mask produced by
                # _dense_mask_from_fn (True = attend); it already encodes the
                # block-causal structure over the 128-padded length.
                x = _neuron_flex_attention(
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _prepare_blockwise_causal_attn_mask(
        ...
        # Neuron: materialize the flex-style attention_mask closure into a dense
        # boolean [Lq, Lk] mask (True = attend) instead of a flex BlockMask.
        block_mask = _dense_mask_from_fn(
            attention_mask, q_len=total_length + padded_length,
